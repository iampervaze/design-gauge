...
export abstract class BaseGauge<
  T extends CanvasGauges.BaseGauge,
  T2 extends CanvasGauges.GenericOptions
> implements OnInit, AfterViewInit {
  ...
  protected listenForDOMEvents() {
    ...
    this.domListener = Rx.DOM.fromMutationObserver(this.el.nativeElement, {
      attributes: true
    }).subscribe(changes => {
      ...
      changes.forEach(change => {
        if ('attributes' === change.type) {
          ...
          newOptions[
            attributeName2PropertyName(change.attributeName)
  ...
  public updateSize(width: number, height: number) {
    ...
    this.el.nativeElement.setAttribute('height', height);
  }
}
